...
Proof.
  ...
  split.
  - apply and_intro. (* Também pode-se usar split *)
    + apply HP.
    + apply HQ.
  - apply HR.
Qed.
...
Proof.
  intros [|n].
  - left. reflexivity.
  - right. Search mult. destruct m eqn: eq.
    + reflexivity.
    + simpl in H. inversion H.
Qed. 
(** [] *)
...
(* 
  Seja uma proposição P qualquer. Temos que ~~P é equivalente a 
  (P->Falso) -> Falso.
  Como temos P, então teremos P->Falso e, pelo princípio da explosão,
  qualquer implicação partindo do Falso é verdadeira. Assim Falso implica Falso,
  e logo, P -> ~~P.
*)
(** [] *)
...
(* 
  Temos que P /\ ~P (and P (not P)) é sempre False. Logo vamos ter:
   ~False = Verdadeiro, e, assim, False -> False. O que é verdadeiro.
 *)
(** [] *)
...
Proof.
  ...
  split.
  - intros H. apply HQ'. apply HP. apply H.
  - intros H. apply HQ. apply HR. apply H.
Qed.
...
Proof.
   ...
   - intros [[x P']|[x' Q']]. 
      + exists x. left. apply P'.
      + exists x'. apply or_commut. apply or_intro. apply Q'.
Qed. 
...
Proof.
  ...
  - simpl. intros y. split.
    + intros [].
    + intros [H H']. destruct H'. apply H1.
  - simpl. split.
    + intros [H | H']. 
      * exists x. split.
        { apply H. }
      ...
      * rewrite -> IHl in H'. 
        ...
        { inversion H. right. apply H1. }
    + intros [x' H]. destruct H as [H1 [H2 | H3]].
      * left. rewrite <- H2 in H1. apply H1.
      * right. apply IHl. exists x'. split. apply H1. apply H3.
Qed.
...
Proof.
  ...
  - simpl. split. 
    ...
    + intros [[] | H]. apply H.
  - simpl. split.
    + intros [H | H']. apply or_assoc.
      * left. apply H.
      * apply or_assoc. rewrite -> IHl in H'. right. apply H'.
    + intros H. apply or_assoc in H. inversion H.
      * left. apply H0.
...
Eval compute in (All (fun x => orb x false = true) [true;true]).

Example All_example_1 : All (fun x => orb x false = true) [true;true].
Proof.
  simpl. split.
  - reflexivity.
  - split.
...
Proof.
  ...
  - split.
    ...
    + intros H. simpl in H. intros x. simpl. apply ex_falso_quodlibet.
  - split.
    + inversion IHl. intros H1. simpl. split.
      * apply H1. simpl. left. reflexivity.
      * apply IHl. intros x0 H2. apply H1. simpl. right. apply H2.
    + intros [H]. 
      rewrite <- IHl in H0.
      simpl.
      intros y. intros H1. inversion H1.
      * rewrite -> H2 in H.
        apply H.
...
Eval compute in (combine_odd_even (fun (n: nat) => n = 4) 
(fun (n: nat) => n = 2) 7).

(** To test your definition, prove the following facts: *)

Lemma evenb_oddb : forall (n: nat), 
  evenb n = negb (oddb n).
Proof.
  destruct n.
  - reflexivity.
...
Proof.
  intros Podd Peven n.
  intros H H'. unfold combine_odd_even. destruct (evenb n) eqn: eq.
  - destruct n eqn: eq1.
    + apply H'. reflexivity.
    + rewrite -> evenb_oddb in eq.
      assert(H'': oddb (S n0) = false).
      { unfold oddb. rewrite -> evenb_oddb. rewrite -> eq. reflexivity. }
      apply H' in H''. apply H''.
  - rewrite -> evenb_oddb in eq.
    assert(H'': oddb n = true).
    { unfold oddb. rewrite -> evenb_oddb. rewrite -> eq. reflexivity. }
    apply H in H''. apply H''.
Qed.
...
Theorem combine_odd_even_elim_odd :
  forall (Podd Peven : nat -> Prop) (n : nat),
    ...
    Podd n.
Proof.
  intros Podd Peven n H H'.
  unfold combine_odd_even in H.
  rewrite -> evenb_oddb in H.
  rewrite -> H' in H.
  simpl in H.
  apply H. 
Qed.
...
Proof.
  intros Podd Peven n H H'.
  unfold combine_odd_even in H.
  assert (H'': evenb n = true).
  { rewrite -> evenb_oddb. rewrite -> H'. reflexivity. }
  rewrite -> H'' in H.
  apply H.
Qed.
(** [] *)
...
(* Fact tr_rev_fact : forall {X} (l l2 : list X), 
...
Proof.
  ...
  - rewrite -> evenb_S. destruct (evenb n) eqn: eq.
    + simpl. inversion IHn. exists x. rewrite -> H. reflexivity.
    + simpl. 
      inversion IHn. 
      exists (S x).
      simpl.
      apply f_equal with (B:=nat) (f:=S) in H.
      apply H.
Qed.
...
Proof.
  ...
  - destruct b1 eqn: eq. 
    + intros H. split.
      ...
      * simpl in H. apply H.
    + intros H. inversion H.
  - intros H. inversion H. rewrite -> H0, H1. reflexivity.
Qed.
...
Proof.
  split.
  - intros H. destruct b1 eqn: eq.
    + left. reflexivity.
    + right. simpl in H. apply H.
  - intros H. inversion H.
    + rewrite -> H0. reflexivity.
    + rewrite -> H0. 
      replace (b1 || true) with (true || b1).
        * reflexivity.
        * destruct b1.
          { reflexivity. } 
          { reflexivity. }
...
Fixpoint beq_list {A : Type} (beq : A -> A -> bool)
  ...
  | hd :: tl => match l2 with
                | [] => false
                | h :: t => if beq hd h then beq_list beq tl t
                            else false 
                end
  end.
...
Proof.
  ...
  - generalize dependent l2. generalize dependent l1. induction l1.
    + intros l2 H1. inversion H1. destruct l2.
       * reflexivity.
       * inversion H2.
    + destruct l2 eqn: eq2. (* simpl in H1. destruct l2 eqn: eq2. *)
      * simpl. intros H1. inversion H1.
      * simpl. 
        { destruct (beq x a) eqn: eq.
          - intros.
            apply IHl1 in H0.
            apply H in eq.  
            rewrite -> eq.
  ...
  - generalize dependent l2. induction l1.
     ...
     + destruct l2.
       * intros H1. inversion H1.
       * intros H1. simpl.
         { destruct (beq x a) eqn: eq_beq.
           - apply H in eq_beq.
             apply IHl1.
             rewrite eq_beq in H1.
             inversion H1.
             reflexivity.
           - inversion H1.
             apply H in H2.
...
Proof.
  intros P. unfold not.
  intros H. apply H. right. intros H'. apply H. left. apply H'.
Qed.
  (** [] *)
...
Proof.
  intros H X P H'. unfold excluded_middle in H. intros x.
  assert(H2: P x \/ ~P x).
  { apply H. }
  unfold not in H'.
  inversion H2.
...
Theorem equivalent_trans : forall P Q R: Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros.
  split.
...
Theorem double_negation_elimination_excluded_middle:
...
Proof.
  ...
  - intros. destruct (H P).
    + apply H1.
    + unfold not in H1. unfold not in H0.
...
Proof.
  ...
  - intros. destruct (H P).
    + left. apply H1.
    + unfold not in H0. destruct (H Q). 
      * right. apply H2.
      * unfold not in H1. unfold not in H2.
        destruct H0.
        { split.
          - apply H1.
          - apply H2.
...
Proof.
  apply equivalent_trans with (P:=de_morgan_not_and_not) (Q:=excluded_middle)
  (R:= implies_to_or).
  - symmetry. apply excluded_middle_de_morgan.
