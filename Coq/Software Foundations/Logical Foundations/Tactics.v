...
(* A tática rewrite apenas reescreve o atual goal de acordo com a hipótese que é
usada nele, enquanto apply tenta mostrar que a igualdade (goal) é verdadeira,
usando a hipótese.
*)
*)
(** [] *)
...
(* Queremos mostrar para todo n, m a seguinte proposição:
    - P(n): " beq_nat n m = true -> n = m. "
    Usando indução em n temos:
    - P(0): "se beq_nat 0 m = true então 0 = m".
      Se m for 0 temos que beq_nat 0 0 = true.
      Se m for diferente de 0 temos que beq_nat 0 m = true é falso, e assim,
      (beq_nat 0 m = true -> 0 = m) = verdadeiro, para todo m.
    - P(n) -> P(S(n)): "Assumindo P(n) temos que provar que P(S(n)) vale."
      Vamos fazer análise de casos em m:
        - Se m = 0, temos beq_nat S(n) 0 = true é falso, e assim,
          (beq_nat S(n) 0 = true -> S(n) = 0) = verdadeiro, para todo m.
        - Se m for diferente de 0, temos que provar que se 
          beq_nat S(n) S(m) = true -> S(n) = S(m). 
          Mas, como  beq_nat S(n) S(m) = true, temos pela H.I. que
          S(n) = S(m), como queríamos.
*)
(** [] *)
...
Proof.
    ...
    - simpl. reflexivity.
    - intros n H. 
      (* assert(H': length(m :: l) = 1 + length l).
      { simpl. reflexivity. } *)
...
Proof.
  ...
  - intros l1 l2. simpl. destruct x.
    + intros H. destruct l1.
      * inversion H.
...
Proof.
  ...
  - simpl. destruct m eqn: eqm.
    + reflexivity.
    + reflexivity.
  - destruct m eqn: eqm2.
    + reflexivity.
    + rewrite <- eqm2.
      destruct (beq_nat (S n) m) eqn: eq3.
      * apply beq_nat_true in eq3.
        rewrite -> eq3.
        apply beq_nat_refl.
      * destruct (beq_nat m (S n)) eqn: eq4.
        { apply beq_nat_true in eq4. 
          rewrite -> eq4 in eq3. 
          inversion eq3.
          symmetry.
          apply beq_nat_refl. }
...
(** Give an informal proof of this lemma that corresponds to your
   ...
   Proof:
   - Primeiro, suponha n = 0. 
     Se m = 0, temos que beq_nat 0 0 = beq_nat m 0 = beq_nat 0 m = verdadeiro. 
     Se m = S m' temos que beq_nat S m' 0 = beq_nat 0 S m' = falso.
   - Se n = S n' suponha m = 0. Teremos que beq_nat S n 0 = beq_nat 0 S n = falso.
     Suponha m = S m' e, depois, vamos analisar beq_nat (S n') m = x.
     Se x = true. Temos então, pela definição de beq_nat que, S n = m.
     Assim, beq_nat m S n' = x = true.
     Seja x = false. Vamos analisar então beq_nat m (S n') = y.
     Suponha y = true, temos que m = S n', assim y = true, e x = true, um absurdo. 
     Logo y deve ser falso. Assim x = y = falso, como queríamos. 
[]
*)
...
Proof.
  ...
  - intros l2 H. destruct l2 eqn: eq.
    + reflexivity.
    + inversion H.
  - intros l2 H. destruct l2 eqn: eq2.
    + inversion H.
...
Proof.
  induction l.
  - intros lf IH. simpl in IH. inversion IH.
  - intros lf H. simpl in H. destruct (test x0) eqn: eq.
    + inversion H. rewrite <- H1. apply eq.
    + generalize dependent lf. apply IHl.
Qed. 
(** [] *)
...
Fixpoint forallb {X: Type} (test: X -> bool) (l: list X) : bool :=
  match l with
  | [] => true
  | head :: tail => andb (test head) (forallb test tail)
  end.
...
Fixpoint existsb {X: Type} (test: X -> bool) (l: list X) : bool :=
  match l with
  | [] => false
  | hd :: tail => orb (test hd) (existsb test tail)
  end.
...
Eval compute in (existsb' (andb true) [true;true;false]).
