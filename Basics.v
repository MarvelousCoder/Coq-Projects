...
Fixpoint factorial (n:nat) : nat :=
  match n with
  | O => 1
  | S n => plus (mult n (factorial(n))) (factorial(n))
  end.
...
Definition blt_nat (n m : nat) : bool :=
  ...
  | S n' => match m with
            | 0 => false
            | S m' => leb n' m' && negb (beq_nat n' m')
            end
  end.
(* Flavio: consegue reescrever esta definição em uma linha, ou seja, 
removendo os doi math..with? *)

(* Challenge Accepted *)
Definition blt_nat_flavio (n m : nat) : bool :=
...
Example test_blt_nat_flavio1: (blt_nat_flavio 2 2) = false.
Proof. simpl. reflexivity. Qed.
Example test_blt_nat_flavio2: (blt_nat_flavio 2 4) = true.
Proof. simpl. reflexivity. Qed.
Example test_blt_nat_flavio3: (blt_nat_flavio 4 2) = false.
Proof. simpl. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - reflexivity.
Qed.
(* Flavio: a tática symmetry faz o trabalho do seu lema equality_c. *)

(* Nem usei esse Lemma. Mas fui testar uma coisa antes, e já que fiz, 
deixa ele aí hehe
*)
...
(* FILL IN HERE *)
(** [] *)
(* Flavio: Seguem algumas dicas e exercícios adicionais. *)
Inductive bin : Type :=
| z : bin 
| d : bin -> bin
| sd : bin -> bin.
...
Compute (incr (sd z)).
Compute (incr (d (sd z))).
Compute (incr (sd (sd z))).
Compute (incr (d (d (sd z)))).

Example test_incr1: (incr z) = sd z.
Proof. simpl. reflexivity. Qed.
Example test_incr2: (incr (sd z)) = d(incr z).
Proof. simpl. reflexivity. Qed.
...
Example bin_to_nat0: (bin_to_nat(z)) = O.
Proof. simpl. reflexivity. Qed.
Example bin_to_nat1: (bin_to_nat(sd z)) = 1.
Proof. simpl. reflexivity. Qed.
Example bin_to_nat2: (bin_to_nat(d (sd z))) = 2.
Proof. simpl. reflexivity. Qed.
Example bin_to_nat3: (bin_to_nat(sd (sd z))) = 3.
Proof. simpl. reflexivity. Qed.

Compute(bin_to_nat (sd (sd z))) = 3.
