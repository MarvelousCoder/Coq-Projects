...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with
  | nil => []
  | h :: tail => match oddb h with
                 | true => h :: oddmembers tail
...
(* match l1 with
   | nil => l2
   | h :: tl => h :: (alternate l2 tl)
   end.
Primeira coisa que pensei, mas falha no requisito do Coq que toda 
definição Fixpoint tem que estar "obviamente decrementando" :(
*)
match l1 with
  | nil => l2
  | h :: tail => match l2 with
               | nil => l1
               | h2 :: tail2 => [h;h2] ++ (alternate tail tail2)
               end
end.
...
(* match v with
   | 0 => length(s) - length(nonzeros s)
   | S v' => match beq_nat (S v') (hd 0 s - 1) with
             | true => 1 + count (S v') (tl (s))
             | false => count (S v') (tl (s))
            end
  end.
Putz, mesmo problema de alternate...
Maldito Coq e seu decreasing argument ¬¬
*)
  match s with
  | nil => 0
  | h :: tail => match beq_nat h v with
                 | true => 1 + (count h tail)
                 | false => count v tail
                 end
 end.
...
Proof. simpl. reflexivity. Qed.
Example test_count3:              count 2 [1;2;3;1;4;1] = 1.
Proof. simpl. reflexivity. Qed.

Eval compute in count 3 [].
Eval compute in count 4 [4;4;4;4;4].
...
Definition member (v:nat) (s:bag) : bool :=
  blt_nat_flavio 0 (count v s). 
(* blt nesse caso retorna true se count v s for maior que 0 e false 
caso contrário *)
...
Fixpoint remove_one (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | h :: tail => match beq_nat v h with
                 | true => tail
                 | false => h :: remove_one v tail
                 end
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | head :: tail => match beq_nat v head with
...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  match s1 with
  | nil => true
  | h :: tail => match member h s2 with
                 | true => subset tail(remove_one h s2)
                 | false => false
                 end
  end.
...
Proof. simpl. reflexivity. Qed.
Example test_subset3:              subset [1;2;4] [2;1;4;1] = true.
Proof. simpl. reflexivity. Qed.
Example test_subset4:              subset [] [2;1;4;1] = true.
Proof. simpl. reflexivity. Qed.
Example test_subset5:              subset [3;2;1;1] [1;2;3;3] = false.
Proof. simpl. reflexivity. Qed.
(** [] *)
...
Fact beq_nat_refl2 : forall n : nat,
  beq_nat n n = true.
Proof.
...
Proof.
  intros v s. destruct v.
  - simpl. reflexivity.
  - simpl. rewrite -> beq_nat_refl2. reflexivity.
Qed.
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  ...
  | h :: tail => match l2 with
                 | nil => false
                 | h2 :: tail2 => beq_nat h h2 && beq_natlist tail tail2
                 end
  end.
...
Example test_beq_natlist4 :
  beq_natlist [4;2;1] [1;2;4] = false.
Proof. simpl. reflexivity. Qed.
...
Proof.
  induction l as [| n l' IHl'].
  - simpl. reflexivity.
  - simpl. rewrite -> beq_nat_refl2. rewrite <- IHl'. reflexivity.
Qed.
(** [] *)
...
(** Write down an interesting theorem [bag_count_sum] about bags
    ...
    [count]!) *)
Theorem bag_count_sum: forall (s : bag) (v : nat),
  leb (count v (sum nil s)) (count v(sum [v] s)) = true.
Proof.
  destruct s.
  - simpl. reflexivity.
  - simpl. intros v. rewrite -> beq_nat_refl2. apply ble_n_Sn.
Qed.
