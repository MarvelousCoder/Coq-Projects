...
(** Use [filter] (instead of [Fixpoint]) to write a Coq function
    ...
    7. *)
Search blt_nat_flavio.

Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun l => evenb l && blt_nat_flavio 7 l) l.
...
Definition partition {X : Type}
  ...
  | [] => ([],[])
  | h :: t => if test h then (h :: (filter test t), 
                                filter(fun t => negb (test t))t)
              else (filter test t, h :: (filter (fun t => negb (test t))t))
  end.
...
Proof. simpl. reflexivity. Qed.
Example test_partition3: partition evenb [1;2;3;4;5] = ([2;4], [1;3;5]).
Proof. simpl. reflexivity. Qed.
(** [] *)
...
Lemma map_distr: forall (X Y : Type) (f : X -> Y) (l l2 : list X),
  map f (l ++ l2) = (map f l) ++ (map f l2).
Proof.
  intros X Y f l l2. induction l.
  - simpl. reflexivity.
  - simpl. rewrite -> IHl. reflexivity.
Qed.
...
*)
Fixpoint map2 (X Y:Type) (f:X->Y) (l:list X) : (list Y) :=
  match l with
  | []     => []
  | h :: t => (f h) :: (map2 _ _ f t)
  end.
...
Fixpoint filter2 (X:Type) (test: X->bool) (l:list X)
                : (list X) :=
  match l with
  | []     => []
  | h :: t => if test h then h :: (filter2 _ test t)
                        else       filter2 _ test t
  end.
...
(* Seria útil caso estivéssemos trabalhando com prod, onde X seria uma lista
de naturais e Y seria do tipo list X*Y, por exemplo. 
*)
(** [] *)
...
Eval compute in (fold_map oddb [1;2;3;4]).
...
(* Lemma curry: forall (X Y Z : Type)
                        (f : X -> Y -> Z) (g : X * Y -> Z) (p : X * Y)
                        x y,
  f x y = prod_curry g x y. 
Proof.
  intros X Y Z f g p x y.
  replace (g) with (prod_uncurry f).
Qed. *)
...
Proof: Por indução em l.
- Primeira suponha l = [], temos que length [] = 0 -> @nth_error X [] n = None.
- Agora suponha que l1 = a :: l, onde: 
  forall X n l, length l = n -> @nth_error X l n = None
Queremos mostrar que: forall X n l1, length l1 = n -> @nth_error X l1 n = None.
length (a :: l) = length l + 1, usando definição de length. Usando que 
length l = n, temos n + 1 = n, o que é falso. Logo a implicação é verdadeira.
*)
(** [] *)
